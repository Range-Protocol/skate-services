...
cd ../../contracts/lib/eigenlayer-contracts

# deployment overwrites this file, so we save it as backup, because we want that output in our local files, 
# and not in the eigenlayer-contracts submodule files
...
##### M2_Deploy_From_Scratch.s.sol prepends "script/deploy/" to the configFile passed as input (M2_deploy_from_scratch.anvil.config.json)
