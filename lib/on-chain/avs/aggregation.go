...
func (a *StdSignatureAggregator) AggregateSignatures(ctx context.Context, state *IndexedOperatorState, quorumIDs []QuorumID, message [32]byte, messageChan chan SignerMessage) (*SignatureAggregation, error) {
	...
	for numReply := 0; numReply < numOperators; numReply++ {
		var err error
		r := <-messageChan
		operatorIDHex := r.Operator.LogValue()
		...
		for idx, quorumID := range quorumIDs {
			...
			// Add to stake signed
			stakeSigned[idx].Add(stakeSigned[idx], opInfo.Stake)
			...
			if aggSigs[idx] == nil {
				...
				aggPubKeys[idx].Add(op.PubkeyG2)
			}
		}
...
func GetStakeThreshold(state *OperatorState, quorum QuorumID, quorumThreshold uint8) *big.Int {
	...
	stakeThreshold = math.RoundUpDivideBig(stakeThreshold, new(big.Int).SetUint64(100))

	return stakeThreshold
}

func GetSignedPercentage(state *OperatorState, quorum QuorumID, stakeAmount *big.Int) uint8 {
	stakeAmount = stakeAmount.Mul(stakeAmount, new(big.Int).SetUint64(100))
